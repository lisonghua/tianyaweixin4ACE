...
//alert("${symbol_dollar}{ctx}/pm/group/group!getUserTreeOfGroup.action");
<!--
jq(function(){
	/*
	jq('${symbol_pound}candidate-users-tree-list').treegrid({
			title:"",
			//width:980,
			height:450,
			//iconCls:'icon-save',
			url : '${symbol_dollar}{ctx}/pm/group/group!getUserTreeOfGroup.action',
			animate : true,
			nowrap : false,
			rownumbers : true,
			collapsible : true,
	...
	jq('${symbol_pound}candidate-users-tree').tree({
		checkbox: true,
		url: '${symbol_dollar}{ctx}/pm/group/group!getUserTreeOfGroup.action',
	...
	*/
	_task_unselected_candidate_user_panel=jq('${symbol_pound}task-unselect-candidate-user-panel').panel({
		border:false,
		noheader:true,
		top:0,
		left:0
	...
	_task_unselect_candidate_user_list=jq('${symbol_pound}task-unselect-candidate-user-list').datagrid({
		title:"Users",
		url:'${symbol_dollar}{ctx}/wf/procdef/procdef!searchCandidateUser.action',
		//singleSelect:true,
		//width:900,
		height:400,
		idField:'userId',
	...
	_task_unselect_candidate_user_group=jq('${symbol_pound}task-unselect-candidate-user-group').combobox({
	    url:'${symbol_dollar}{ctx}/pm/group/group!getAllGroups.action',
	    //editable:false,
	    valueField:'groupId',
	    textField:'name'
	});
...
function searchCandidateUser(){
	var name=jq("${symbol_pound}task-unselect-candidate-user-userName").val();
	var sso=jq("${symbol_pound}task-unselect-candidate-user-sso").val();
	var group=_task_unselect_candidate_user_group.combobox('getValue');
	//alert(group);
	var email=jq("${symbol_pound}task-unselect-candidate-user-email").val();
	//alert(sso);
...
<table border="0">
	<tr>
		...
		<td><a href="${symbol_pound}" class="easyui-linkbutton" iconCls="icon-search" onclick="searchCandidateUser();">Search</a></td>
	</tr>
</table>
