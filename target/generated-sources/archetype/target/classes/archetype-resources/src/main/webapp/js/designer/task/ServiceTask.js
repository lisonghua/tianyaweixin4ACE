...
draw2d.ServiceTask.prototype.parseServiceXML=function(jqObject){
	var clazz=jq(jqObject).attr("activiti:class");
	var expression=jq(jqObject).attr("activiti:expression");
	var delegateExpression=jq(jqObject).attr("activiti:delegateExpression");
	if(clazz!=null&&clazz!=""){
		this._type="javaClass";
		this._javaClass=clazz;
	}else if(expression!=null&&expression!=""){
		this._type="expression";
		this._expression=expression;
	}else if(delegateExpression!=null&&delegateExpression!=""){
		this._type="delegateExpression";
		this.delegateExpression=delegateExpression;
	}
	this.resultVariable=jq(jqObject).attr("activiti:resultVariableName");
};
...
draw2d.ServiceTask.prototype.toObject=function(jqObject){
	this.parseTaskGeneralXML(jqObject);
	this.parseServiceXML(jqObject);
	this.parseDocumentationXML(jqObject);
	this.parseFieldsXML(jqObject);
