...
${symbol_dollar}(function(){
	_group_panel_obj=${symbol_dollar}('${symbol_pound}_group_panel').panel({
			//height:600,
			border:false,
			noheader:true,
			top:0,
	...
	_group_grid=${symbol_dollar}('${symbol_pound}_group_table').datagrid({
		title:"<s:text name='label.pm.group.list'></s:text>",
		url:'${symbol_dollar}{ctx}/pm/group/group!search.action',
		//singleSelect:true,
		idField:'groupId',
		height:500,
		pagination:true,
	    ...
	    toolbar:[{
	        ...
	        iconCls:'icon-add',
	        handler:function(){
	    		window.location='${symbol_dollar}{ctx}/pm/group/group!forInsert.action';
				return false;
	        }
	    }]
	});
...
function  editrow(id){
	window.location='${symbol_dollar}{ctx}/pm/group/group!forInsert.action?groupId='+id;
}
function  deleterow(id){
	${symbol_dollar}.messager.confirm("Confirm","<s:text name='message.common.delete.confirm'></s:text>",function(flag){
		if(flag){
			${symbol_dollar}.ajax({
					url:'${symbol_dollar}{ctx}/pm/group/group!delete.action?_group_id='+id,
					type: 'GET',
					dataType:'json',
					error:function(){
...
function displayActionBt (value,rowData,rowIndex){
        ...
        var a = '<img onclick="addUsers('+id+')" src="${symbol_dollar}{ctx}/image/assign.gif" title="'+"<s:text name='label.pm.group.add.user'></s:text>"+'" style="cursor:hand;"/>';
        return e+'&nbsp;'+a+'&nbsp;'+d;
}
function addUsers(id){
	window.location="${symbol_dollar}{ctx}/pm/group/group!forAssignUser.action?groupId="+id;
}
//-->
</script>
<body>
