...
${symbol_dollar}(function(){
	...
	_list_grid_obj=${symbol_dollar}('${symbol_pound}_list_table').datagrid({
		title:'',
		url:'${symbol_dollar}{ctx}/genpact/definition/definition!search.action',//加载表格数据的URL
		singleSelect:true,
		height:400,
		idField:'rpDefId',
		pagination:true,
	    ...
	    toolbar:[{
	        ...
	        handler:function(){
	        	//点击添加按钮的URL
	    		window.location='${symbol_dollar}{ctx}/genpact/definition/definition!forInsert.action';
	    		return false;
	        }
	    }]
	});
...
//点击修改按钮转向的URL
function  editRow(id){
	window.location='${symbol_dollar}{ctx}/genpact/definition/definition!forUpdate.action?rpDefId='+id;
}
//点击删除按钮的URL
function  deleteRow(id){
	${symbol_dollar}.messager.confirm("Confirm","<s:text name='message.common.delete.confirm'></s:text>",function(flag){
		if(flag){
			${symbol_dollar}.ajax({
					url:'${symbol_dollar}{ctx}/genpact/definition/definition!delete.action?rpDefId='+id,//删除调用的URL
					type: 'GET',
					dataType:'json',
					error:function(){
