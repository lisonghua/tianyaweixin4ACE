...
${symbol_dollar}(function(){
	_opt_panel_obj=${symbol_dollar}('${symbol_pound}_opt_panel').panel({
			//height:600,
			border:false,
			noheader:true,
			top:0,
	...
	_opt_grid=${symbol_dollar}('${symbol_pound}_opt_table').datagrid({
		title:"<s:text name='label.pm.opt.list'></s:text>",
		url:'${symbol_dollar}{ctx}/pm/opt/operation!search.action',
		//singleSelect:true,
		height:500,
		idField:'operationId',
		pagination:true,
	    ...
	    toolbar:[{
	        ...
	        iconCls:'icon-add',
	        handler:function(){
	    		window.location='${symbol_dollar}{ctx}/pm/opt/operation!forInsert.action';
				return false;
	        }
	    }]
	});
...
function  editrow(id){
	window.location='${symbol_dollar}{ctx}/pm/opt/operation!forInsert.action?operationId='+id;
}
function  deleterow(id){
	${symbol_dollar}.messager.confirm("Confirm","<s:text name='message.common.delete.confirm'></s:text>",function(flag){
		if(flag){
			${symbol_dollar}.ajax({
					url:'${symbol_dollar}{ctx}/pm/opt/operation!delete.action?_operation_id='+id,
					type: 'GET',
					dataType:'json',
					error:function(){
