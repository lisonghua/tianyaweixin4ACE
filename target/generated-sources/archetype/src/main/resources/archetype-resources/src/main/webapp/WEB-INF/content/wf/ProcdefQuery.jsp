...
function  viewProcinst(id){
	window.location='${symbol_dollar}{ctx}/wf/procdef/procdef!forInstance.action?id='+id;
}
...
function  viewProcDiag(id){
	...
	window.open('${symbol_dollar}{ctx}/wf/procdef/procdef!forDiagram.action?id='+id,"diagram","width=1500px,height=800px,directories=0,menubar=0,resizable=0,status=0,titlebar=0,toolbar=0");
}
//输出每一行末尾的操作按钮
function actionFormatter(value,rowData,rowIndex){
	var id = rowData.id;
	var v = '<img onclick="viewProcinst(${symbol_escape}''+id+'${symbol_escape}')" src="${symbol_dollar}{ctx}/image/zoom.png" title="'+"<s:text name='button.wf.process.instance.view'></s:text>"+'" style="cursor:hand;"/>';   
    var vd = '<img onclick="viewProcDiag(${symbol_escape}''+id+'${symbol_escape}')" src="${symbol_dollar}{ctx}/image/picture_go.png" title="'+"<s:text name='button.wf.process.diagram.view'></s:text>"+'" style="cursor:hand;"/>';
    return v+'&nbsp;'+vd;
}
//点击查询按钮的操作函数
