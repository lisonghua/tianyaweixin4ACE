...
${symbol_dollar}(function(){
	...
	_list_grid_obj=${symbol_dollar}('${symbol_pound}_list_table').datagrid({
		...
		url:'${symbol_dollar}{ctx}/sm/server/server!search.action',
		singleSelect:true,
		height:500,
		//idField:'serverId',
		pagination:true,
	    ...
	    toolbar:[{
	        ...
	        iconCls:'icon-add',
	        handler:function(){
	    		window.location='${symbol_dollar}{ctx}/sm/server/server!forUpdate.action';
	    		return false;
	        }
	    }]
	});
...
function  editRow(id){
	window.location='${symbol_dollar}{ctx}/sm/server/server!forUpdate.action?serverId='+id;
}
function  deleteRow(id){
	${symbol_dollar}.messager.confirm("Confirm","<s:text name='message.common.delete.confirm'></s:text>",function(flag){
		if(flag){
			${symbol_dollar}.ajax({
					url:'${symbol_dollar}{ctx}/sm/server/server!delete.action?serverId='+id,
					type: 'GET',
					dataType:'json',
					error:function(){
...
function actionFormatter(value,rowData,rowIndex){
    ...
    var c = '<img onclick="viewResources('+id+')" src="${symbol_dollar}{ctx}/image/pages.gif" title="'+"<s:text name='button.sm.server.view.resources'></s:text>"+'" style="cursor:hand;"/>';
    return c+'&nbsp;'+e+'&nbsp;'+d;
}
function viewResources(id){
	window.location="${symbol_dollar}{ctx}/sm/server/server!forView.action?serverId="+id;
}
