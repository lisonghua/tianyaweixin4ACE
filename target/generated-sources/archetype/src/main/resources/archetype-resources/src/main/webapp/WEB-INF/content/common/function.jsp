...
${symbol_dollar}(function(){
	funcTab=${symbol_dollar}('${symbol_pound}functionGroup').tabs({
		fit:true
		});
});
function goHome(){
	...
	if(!isExistTab(homeTitle)){
		funcTab.tabs('add',{
			title: homeTitle,
			fit:true,
			href:'${symbol_dollar}{ctx}/login/forword!forwardFramePage.action?_f_url=${symbol_dollar}{ctx}/login/forword!forwardHomePage.action',
...
function addTab(title,path){
	funcTab.tabs('add',{
		title: title,
		fit:true,
		href:'${symbol_dollar}{ctx}/login/forword!forwardFramePage.action?_f_url='+path,
		iconCls:'icon-save',
		closable:true
	});
}
...
<div id="functionGroup">
    <div title="<s:text name='label.common.homepage'></s:text>" iconCls="homepage-banner-bt-home" closable="false" style="overflow:hidden;padding:0px;">
       <iframe frameborder="0" height="100%" marginheight="0" marginwidth="0" width="100%" src="${symbol_dollar}{ctx}/login/forword!forwardHomePage.action" onload="onloadSwitchStyle()"></iframe>
    </div>
</div>
