...
${symbol_dollar}(function(){
	...
	_list_grid_obj=${symbol_dollar}('${symbol_pound}_list_table').datagrid({
		title:'<s:text name="label.sm.connectionpool.table.title"></s:text>(${symbol_dollar}{ip})',
		url:'${symbol_dollar}{ctx}/sm/connectionpool/connectionpool!search.action?serverId=${symbol_dollar}{serverId}',
		singleSelect:true,
		height:500,
	    ...
	    toolbar:[{
	        ...
	        iconCls:'icon-add',
	        handler:function(){
	    		window.location='${symbol_dollar}{ctx}/sm/connectionpool/connectionpool!forInsert.action?serverId=${symbol_dollar}{serverId}';
	    		return false;
	        }
	    },'-',
	    {
	...
	_dialog_win_obj = ${symbol_dollar}('${symbol_pound}_dialog_win').window({   
	    closed:true,
	    //draggable:false,
	    //collapsible:false,
	    minimizable:false,
...
function  editRow(id){
	window.location='${symbol_dollar}{ctx}/sm/connectionpool/connectionpool!forUpdate.action?serverId=${symbol_dollar}{serverId}&connectionpoolId='+id;
}
function  deleteRow(id){
	${symbol_dollar}.messager.confirm("Confirm","<s:text name='message.common.delete.confirm'></s:text>",function(flag){
		if(flag){
			${symbol_dollar}.ajax({
					url:'${symbol_dollar}{ctx}/sm/connectionpool/connectionpool!delete.action?connectionpoolId='+id,
					type: 'GET',
					dataType:'json',
					error:function(){
...
function actionFormatter(value,rowData,rowIndex){
    ...
    var c = '<img onclick="viewConnectionPool('+id+')" src="${symbol_dollar}{ctx}/image/pages.gif" title="'+"<s:text name='button.sm.connectionpool.view.connectionpool'></s:text>"+'" style="cursor:hand;"/>';
    return c+'&nbsp;'+e+'&nbsp;'+d;
}
function viewConnectionPool(id){
	_dialog_win_obj.window('open');
	_dialog_frame_obj.src='${symbol_dollar}{ctx}/sm/connectionpool/connectionpool!forView.action?connectionpoolId='+id;
}
function closeWin(){
...
<div id="_dialog_win" title="<s:text name='label.sm.connectionpool.info'></s:text>(${symbol_dollar}{ip})" style="width:650px;height:400px;">  
