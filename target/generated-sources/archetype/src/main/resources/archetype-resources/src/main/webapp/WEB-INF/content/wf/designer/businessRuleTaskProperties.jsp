...
function saveTaskMainConfig(){
	task.ruleName=jq('${symbol_pound}ruleName').val();
	task.inputVariable=jq('${symbol_pound}inputVariable').val();
	task.resultVariable=jq('${symbol_pound}resultVariable').val();
	if(jq('${symbol_pound}excludeYes').attr('checked'))
		task.excluded=true;
	if(jq('${symbol_pound}excludeNo').attr('checked'))
		task.excluded=false;
}
...
function loadTaskMainConfig(){
	jq('${symbol_pound}ruleName').val(task.ruleName);
	jq('${symbol_pound}inputVariable').val(task.inputVariable);
	jq('${symbol_pound}resultVariable').val(task.resultVariable);
	if(task.excluded)
		jq('${symbol_pound}excludeYes').attr('checked',true);
	else
		jq('${symbol_pound}excludeNo').attr('checked',true);
}
			...
			<div id="mainConfig" title="Main Config" class="properties-menu">
				<table id="main-properties">
					<tr>
						...
						<td>
							<input type="text" id="ruleName" name="ruleName" size="50"/>
						</td>
					</tr>
					<tr>
						<td align="right">Input Variable:</td>
						<td>
							<input type="text" id="inputVariable" name="inputVariable" size="50"/>
						</td>
					</tr>
					<tr>
						<td align="right">Excluded:</td>
						<td>
							<input type="radio" id="excludeYes" name="excluded" value="true"/>Yes
							<input type="radio" id="excludeNo" name="excluded" value="false" checked/>No
						</td>
					</tr>
					<tr>
						<td align="right">Result Variable:</td>
						<td>
							<input type="text" id="resultVariable" name="resultVariable" size="50"/>
						</td>
					</tr>
				</table>
			</div>
