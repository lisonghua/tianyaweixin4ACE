...
jq(function(){
	...
	_task_fields_dg = jq('${symbol_pound}task-Fields-list').datagrid({
		//title:"Listener",
	    ...
	    toolbar:[{
	        ...
	        handler:function(){
				...
				jq('${symbol_pound}task-Fields-list').datagrid('appendRow',{
					id:'',
					fieldName:'',
					type:'',
					value:'',
				...
				var index = jq('${symbol_pound}task-Fields-list').datagrid('getRows').length-1;
				jq('${symbol_pound}task-Fields-list').datagrid('beginEdit', index);
	        }
	    }],
...
function saveTaskMainConfig(){
	if(jq('${symbol_pound}javaClassType').attr('checked')){
		task._type=jq('${symbol_pound}javaClassType').val();
		task._javaClass=jq('${symbol_pound}serviceClass').val();
	}else if(jq('${symbol_pound}expressionType').attr('checked')){
		task._type=jq('${symbol_pound}expressionType').val();
		task._expression=jq('${symbol_pound}_expression').val();
	}else if(jq('${symbol_pound}delegateExpressionType').attr('checked')){
		task._type=jq('${symbol_pound}delegateExpressionType').val();
		task.delegateExpression=jq('${symbol_pound}delegateExpression').val();
	}
	task.resultVariable=jq('${symbol_pound}resultVariable').val();
...
function loadTaskMainConfig(){
	...
	if(task._type=='javaClass'){
			jq('${symbol_pound}serviceClass').val(task._javaClass);
	}else if(task._type=='expression'){
			jq('${symbol_pound}_expression').val(task._expression);
	}else if(task._type=='delegateExpression'){
			jq('${symbol_pound}delegateExpression').val(task.delegateExpression);
	}
	jq('${symbol_pound}resultVariable').val(task.resultVariable);
	loadTaskFields();
...
function loadTaskFields(){
	...
	jq('${symbol_pound}task-Fields-list').datagrid('loadData',field_grid_data);
}
...
function controlTaskTypeDisplay(t){
		if(t=='javaClass'){
			jq('${symbol_pound}serviceClassTr').show();
			jq('${symbol_pound}expressionTr').hide();
			jq('${symbol_pound}delegateExpressionTr').hide();
		}else if(t=='expression'){
			jq('${symbol_pound}serviceClassTr').hide();
			jq('${symbol_pound}expressionTr').show();
			jq('${symbol_pound}delegateExpressionTr').hide();
		}else if(t=='delegateExpression'){
			jq('${symbol_pound}serviceClassTr').hide();
			jq('${symbol_pound}expressionTr').hide();
			jq('${symbol_pound}delegateExpressionTr').show();
		}
}
//-->
</script>
