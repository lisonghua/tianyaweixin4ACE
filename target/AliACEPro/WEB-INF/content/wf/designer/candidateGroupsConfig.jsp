	...
	_task_candidate_groups_list=jq('#task-candidate-groups-list').datagrid({
		...
		toolbar:[{
	        ...
	        handler:function(){
				...
				_task_candidate_win.window('refresh','${ctx}/wf/procdef/procdef!forAddTaskCandidateGroup.action');
	        }
	    }]
	});
...
function candidateGroupsBtFormatter(value,rowData,rowIndex){
    ...
    var d = '<img onclick="deleteCandidateGroup(\''+name+'\')" src="${ctx}/image/delete.gif" title="'+"<s:text name='button.common.delete'></s:text>"+'" style="cursor:hand;"/>';
	return d;
}
