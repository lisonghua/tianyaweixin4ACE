	...
	jq('#task-listeners-list').datagrid({
	    ...
	    toolbar:[{
	        ...
	        handler:function(){
	    		...
	    		//_listener_frame.src="";
	    		listenerId=null;
	    		_listener_win.window('refresh','${ctx}/wf/procdef/procdef!forTaskListenerConfigPage.action');
	...
	jq('#task-form-properties-list').datagrid({
	    ...
	    toolbar:[{
	        ...
	        handler:function(){
	        	...
	        	formPropertyId=null;
	    		_form_win.window('refresh','${ctx}/wf/procdef/procdef!forTaskFormConfigPage.action');
	        }
	    }]
	});
...
function populateTaskProperites(){
	...
	jq('#performerType').combobox('select',task.performerType);
	//alert(task.isUseExpression);
	if(task.isUseExpression=='true'){
		jq('#isExpression').attr('checked','checked')
		jq('#expressionTr').show();
		jq('#expression').val(task.expression);
	}else{
		jq('#nonExpression').attr('checked','checked');
		jq('#expressionTr').hide();
	}
