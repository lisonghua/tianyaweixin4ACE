...
draw2d.BusinessRuleTask.prototype.parseMainConfigXML=function(jqObject){
	this.inputVariable=jq(jqObject).attr("activiti:ruleVariablesInput");
	this.ruleName=jq(jqObject).attr("activiti:rules");
	this.resultVariable=jq(jqObject).attr("activiti:resultVariable");
	this.excluded=jq(jqObject).attr("activiti:exclude");
};
...
draw2d.BusinessRuleTask.prototype.toObject=function(jqObject){
	this.parseTaskGeneralXML(jqObject);
	this.parseMainConfigXML(jqObject);
