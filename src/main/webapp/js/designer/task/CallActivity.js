...
draw2d.CallActivity.prototype.parseCallXML=function(jqObject){
	this.callElement=jq(jqObject).attr("calledElement");
};
...
draw2d.CallActivity.prototype.toObject=function(jqObject){
	this.parseTaskGeneralXML(jqObject);
	this.parseCallXML(jqObject);
	this.parseExecutionListenerXML(jqObject);
	this.parseInputParamsXML(jqObject);
	this.parseOutputParamsXML(jqObject);
...
draw2d.CallActivity.Parameter.prototype.getStartElementXML=function(){
	...
	if(this.sourceExpression!=null&&this.sourceExpression!='')
...
draw2d.CallActivity.Parameter.prototype.toObject=function(jqObject){
	this.source=jq(jqObject).attr("source");
	this.sourceExpression=jq(jqObject).attr("sourceExpression");
	this.target=jq(jqObject).attr("target");
}
