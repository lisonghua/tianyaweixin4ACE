	...
	_task_candidate_users_list=jq('#task-candidate-users-list').datagrid({
		...
		toolbar:[{
	        ...
	        handler:function(){
				...
				_task_candidate_win.window('refresh','${ctx}/wf/procdef/procdef!forAddTaskCandidateUser.action');
	        }
	    }]
	});
...
function candidateUsersBtFormatter(value,rowData,rowIndex){
    ...
    var d = '<img onclick="deleteCandidateUser(\''+sso+'\')" src="${ctx}/image/delete.gif" title="'+"<s:text name='button.common.delete'></s:text>"+'" style="cursor:hand;"/>';
	return d;
}
